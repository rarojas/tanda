...
export default function fieldValidation (state, action) {
  ...
  switch (field) {
    ...
    case ('name'):
      let validName = _.isUndefined(validate({username: value},usernameConstraints))
        if (validName) {
      ...
      case ('mount'):
        let validMount = _.isUndefined(validate({mount: value }, mountConstraints))
          if (validMount) {
            return state.setIn(['form', 'fields', 'mountHasError'],
            false)
            .setIn(['form', 'fields', 'mountErrorMsg'], '')
          } else {
            return state.setIn(['form', 'fields', 'mountHasError'], true)
            .setIn(['form', 'fields', 'mountErrorMsg'],
            I18n.t('FieldValidation.valid_mount'))
        }
