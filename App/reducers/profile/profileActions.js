...
export function getTandaRequest () {
  return {
    type: GET_TANDA_REQUEST
  }
}
export function getTandaSuccess (json) {
  return {
    type: GET_TANDA_SUCCESS,
    payload: json
  }
}
export function getTandaFailure (json) {
  return {
    type: GET_TANDA_FAILURE,
    payload: json
  }
}


export function getTandas (sessionToken) {
  return dispatch => {
    dispatch(getTandaRequest())
    return appAuthToken.getSessionToken(sessionToken)
      .then((token) => {
        return BackendFactory(token).getTandas()
      })
      .then((json) => {
        dispatch(getTandaSuccess(json))
      })
      .catch((error) => {
        dispatch(getTandaFailure(error))
      })
  }
}
/**
